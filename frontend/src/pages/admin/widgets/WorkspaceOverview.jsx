...
export const WorkspaceOverview = ({ kpis, recentActivities, admissionsTrend = [] }) => {
  ...
  const trendData = admissionsTrend.length > 0 ? admissionsTrend : [
    { month: 'Last 6M', count: kpis?.totalAdmissions || 0 }
  ];
    ...
    <div className="space-y-6">
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div variants={item} className="bg-white rounded-3xl p-6 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group">
           ...
           <span className="inline-block mt-3 px-2 py-1 bg-green-50 text-green-600 text-xs font-bold rounded-md">{kpis?.studentGrowth || '0%'} Growth</span>
        </motion.div>
        ...
        <motion.div variants={item} className="bg-white rounded-3xl p-6 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group">
           ...
           <span className="inline-block mt-3 px-2 py-1 bg-blue-50 text-blue-600 text-xs font-bold rounded-md">{kpis?.uniqueVisitors || 0} Unique Visitors</span>
        </motion.div>
        ...
        <motion.div variants={item} className="bg-white rounded-3xl p-6 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group">
           ...
           <p className="text-4xl font-black text-gray-900">{kpis?.conversionRate || 0}%</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Live Admissions Velocity Chart */}
        <div className="lg:col-span-2 bg-white rounded-3xl p-6 border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] h-96 flex flex-col justify-between">
           <div className="flex justify-between items-center mb-6">
             <div>
               <h3 className="text-lg font-bold text-gray-900">Admissions & Enrollment Velocity</h3>
               <p className="text-xs text-gray-400 mt-0.5">Real-time monthly application intake</p>
             </div>
             <span className="flex items-center gap-2 text-xs font-semibold text-green-600 bg-green-50 px-3 py-1 rounded-full border border-green-100">
               <span className="h-2 w-2 rounded-full bg-green-500 animate-pulse"></span> Live Sync
             </span>
           </div>
           
           {/* Real Monthly Admission Trend Chart */}
           <div className="flex-1 flex items-end gap-3 mt-4">
              {trendData.map((item, i) => {
                const heightPct = Math.max((item.count / maxTrend) * 100, 8);
                return (
                  <div key={i} className="flex-1 bg-gray-50 hover:bg-primary-50 rounded-xl transition-all duration-300 cursor-pointer group relative flex flex-col justify-end items-center h-full p-2">
                    <span className="text-xs font-bold text-primary-700 opacity-0 group-hover:opacity-100 transition-opacity mb-1">{item.count}</span>
                    <div 
                      className="w-full rounded-lg transition-all duration-500 bg-gradient-to-t from-primary-600 to-primary-400 group-hover:from-primary-700 group-hover:to-primary-500" 
                      style={{ height: `${heightPct}%` }}
                    />
                    <span className="text-[11px] font-bold text-gray-500 mt-2">{item.month}</span>
                  </div>
                );
              })}
           </div>
