...
export const FacultyDashboard = () => {
  ...
  const popularPrograms = d.popularPrograms || [];
            ...
            <div className="sticky top-28 space-y-6">
              ...
              <nav className="bg-white rounded-3xl p-4 shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 flex flex-col gap-1">
                ...
                <Link to="/programs" className="flex items-center justify-between px-4 py-3 rounded-2xl text-gray-600 hover:bg-gray-50 font-medium transition-all group">
                  <div className="flex items-center gap-3"><BookOpen className="w-5 h-5 text-gray-400 group-hover:text-primary-600 transition-colors" /> Academic Catalog</div>
                </Link>
          ...
          <main className="flex-1 min-w-0">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
                ...
                <p className="text-gray-500 mt-2 text-lg font-medium">Live academic control center.</p>
              </motion.div>
            </div>
            ...
            <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              ...
              <motion.div variants={item} className="col-span-1 md:col-span-2 xl:col-span-2 bg-gradient-to-br from-gray-900 to-gray-800 rounded-3xl p-6 md:p-8 text-white relative overflow-hidden shadow-[0_20px_40px_rgba(15,17,20,0.1)]">
                ...
                <div className="relative z-10 h-full flex flex-col justify-between">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xl font-bold flex items-center gap-2"><BookOpen className="w-5 h-5 text-primary-400" /> Active Course Modules</h3>
                    <span className="text-sm font-semibold bg-white/10 px-3 py-1 rounded-full border border-white/20">{courses.length} Courses</span>
                  </div>
                  <div className="space-y-4">
                    {courses.length > 0 ? (
                      courses.slice(0, 3).map((cls, idx) => (
                        <div key={cls._id || idx} className="bg-white/5 border border-white/10 rounded-2xl p-4 hover:bg-white/10 transition-colors cursor-pointer group">
                          <div className="flex justify-between items-start">
                            <div>
                              <h4 className="font-bold text-white group-hover:text-primary-300 transition-colors">{cls.title || cls.name}</h4>
                              <p className="text-sm text-gray-400 mt-1 flex items-center gap-2"><Clock className="w-4 h-4"/> {cls.duration || 'Semester Module'}</p>
                            </div>
                            <span className="bg-primary-500/20 text-primary-300 text-xs font-bold px-2 py-1 rounded-lg">{cls.level || 'Active'}</span>
                          </div>
                        </div>
                      ))
                    ) : (
                      <p className="text-sm text-gray-400 py-6 text-center">No active courses registered.</p>
                    )}
                  </div>
                </div>
              </motion.div>

              {/* Popular Programs Pipeline (Spans 2 cols) */}
              <motion.div variants={item} className="col-span-1 md:col-span-2 xl:col-span-2 bg-white rounded-3xl p-6 border border-gray-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)]">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                    <TrendingUp className="w-5 h-5 text-primary-600" /> Program Application Intake
                  </h3>
                  <Link to="/programs" className="text-xs font-semibold text-primary-600 hover:underline">View Catalog</Link>
                </div>
                <div className="space-y-4">
                  {popularPrograms.length > 0 ? (
                    popularPrograms.map((prog, idx) => (
                      <div key={idx} className="border border-gray-100 rounded-2xl p-4 hover:border-primary-200 hover:bg-primary-50/20 transition-all group">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h4 className="font-bold text-gray-900">{prog.name}</h4>
                          </div>
                          <span className="bg-primary-50 text-primary-700 text-xs font-bold px-2.5 py-1 rounded-full">{prog.applications} applicants</span>
                        </div>
                      </div>
                    ))
                  ) : (
                    <p className="text-sm text-gray-400 py-8 text-center">No application intake records yet.</p>
                  )}
                </div>
              </motion.div>
              ...
              <motion.div variants={item} className="col-span-1 xl:col-span-1 bg-white rounded-3xl p-6 border border-gray-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] flex flex-col justify-between group hover:border-primary-200 transition-colors">
                <div>
                  ...
                  <h3 className="text-gray-500 font-medium text-sm">Total Enrolled Students</h3>
                  <div className="mt-1">
                    <span className="text-3xl font-black text-gray-900 tracking-tight">{d.totalStudents || 0}</span>
                  </div>
                  <p className="text-xs text-gray-400 mt-2">Active across all programs</p>
                </div>
                <Button onClick={() => navigate('/admin/users')} variant="outline" className="w-full mt-6 rounded-xl border-gray-200 text-indigo-600 hover:bg-indigo-50 hover:border-indigo-200">View Roster</Button>
              </motion.div>

              {/* Real-time System Status (Spans 3 cols) */}
              <motion.div variants={item} className="col-span-1 md:col-span-2 xl:col-span-3 bg-white rounded-3xl p-6 border border-gray-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)]">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                    <Activity className="w-5 h-5 text-primary-600" /> Academic Statistics Overview
                  </h3>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-4 bg-gray-50 rounded-2xl">
                    <p className="text-xs font-bold text-gray-500 uppercase">Registered Programs</p>
                    <p className="text-2xl font-black text-gray-900 mt-1">{d.totalPrograms || 0}</p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-2xl">
                    <p className="text-xs font-bold text-gray-500 uppercase">Active Courses</p>
                    <p className="text-2xl font-black text-gray-900 mt-1">{d.totalCourses || 0}</p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-2xl">
                    <p className="text-xs font-bold text-gray-500 uppercase">Faculty Network</p>
                    <p className="text-2xl font-black text-gray-900 mt-1">Live</p>
                  </div>
                </div>
              </motion.div>
